...
export default function CalculatorForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    setFuelChartData([{ name: "Fuel Cost", Petrol: petrolCost, CNG: cngCost }]);
    setSavingsChartData([
      {
        name: "Savings",
        Monthly: data.monthlySavings,
        Yearly: data.yearlySavings,
      },
    ]);
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        <div>
          ...
          <input
            ...
            inputMode="decimal"
            placeholder="₦0.00"
            className="w-full p-3 rounded-md border text-gray-700 border-gray-300 focus:outline-none focus:ring-2 focus:ring-green-400"
            required
            onFocus={(e) => {
        ...
        <div className="mt-8">
          <div className="text-center">
            <p className="text-lg text-green-700 font-medium">
              💸 Monthly Savings: {formatNaira(monthlySavings)}
            </p>
            <p className="text-lg text-blue-700 font-medium">
              📆 Yearly Savings: {formatNaira(yearlySavings)}
            </p>
          </div>
          ...
          <div className="mt-6 h-[300px]">
            {(activeTab === "fuel" ? fuelChartData : savingsChartData).length >
            0 ? (
