...
export default function CalculatorForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch("/api/calculate", {
      ...
      body: JSON.stringify({
        fuelCost: Number(parseNaira(fuelCost)),
        fuelUsage: Number(parseNaira(fuelUsage)),
      }),
    });
  ...
  // Handle input change and format as Naira
  const handleFuelCostChange = (e) => {
    const raw = parseNaira(e.target.value);
    setFuelCost(raw ? formatNaira(raw) : "");
  };

  const handleFuelUsageChange = (e) => {
    const raw = parseNaira(e.target.value);
    setFuelUsage(raw ? formatNaira(raw) : "");
  };
